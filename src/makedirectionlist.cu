...
//this method finds the matching score between each sequence in the input sequences and itself
//and saves it in result array provided in arg parameter. Both forward and reverse scores are found.
static void	*selfdpthread( void *arg )
{
	...
	while( 1 )
	{
		...
		sreverse( revseq[0], seq[j] ); //defined in io.c. fill revseq[0] with reversed chars from seq[j]
#if GLOBAL
...
#else
		//this method returns max weight between seq+j and seq+j
		res[j] =  L__align11_noalign( n_dis_consweight_multi, seq+j, seq+j ); //defined in Lalign11.c
		//this method returns max weight between seq+j and revseq
		res[j] -= L__align11_noalign( n_dis_consweight_multi, seq+j, revseq );
#endif
	}

	//free allocated memory
	creverse( 0 ); //defined in io.c. this call frees allocated memory from previous call.
	FreeCharMtx( revseq );
#if GLOBAL
...
//					fprintf( stderr, "poffset = %d\n", poffset );
				...
				case 'j':
					...
					fprintf( stdout, "jtt/kimura %d\n", pamN );
					--argc;
					goto nextoption;
				case 't':
					thresholdtorev = atof( *++argv );
					fprintf( stdout, "thresholdtorev = %f\n", thresholdtorev );
					--argc;
					goto nextoption;
				case 'o':
					mode = *(*++argv);
...
//set pointt to some value based on calculations on values in n
...
//returns value based on comparisons of table and pointt values
...
//fills order array with sorted matching scores calculated from aligning each sequence with itself
//using mafft-distance
static void makecontrastorder6mer( int *order, int **pointt, int **pointt_rev, char **seq, int iend, int shift )
{
	int i;
	double *res;
	contrastarr *arr; //defined here
	short *table1, *table1_rev;

	arr = (contrastarr *) calloc( iend, sizeof( contrastarr ) );
	res = (double *) calloc( iend, sizeof( double ) );
	...
	for( i=0; i<iend; i++ )
	{
		...
		make_distance_makecompositiontable_p( table1, pointt[i] ); //defined in mafft-distance.c. fill table1 with values based on pointt[i]
		res[i] = localcommonsextet_p2( table1, pointt[i] ); //defined here. returns value based on comparisons of table1 and pointt[i] values
		free( table1 );

		table1_rev = (short *)calloc( tsize, sizeof( short ) );
		if( !table1_rev ) ErrorExit( "Cannot allocate table1\n" );
		make_distance_makecompositiontable_p( table1_rev, pointt_rev[i] ); //fill table1_rev with values based on pointt_rev[i]
		res[i] -= localcommonsextet_p2( table1_rev, pointt[i] ); //returns value based on comparisons of table1_rev and pointt[i] values
		free( table1_rev );
...
//fills order array with sorted matching scores calculated from aligning each sequence with itself
//using local aligning algorithm
...
#endif
	{
		selfdpthread_arg_t *targ; //defined here
		targ = (selfdpthread_arg_t *) calloc( 1, sizeof( selfdpthread_arg_t ) );
		targ[0].iend = iend;
		targ[0].seq = seq;
		targ[0].res = res;
		selfdpthread( targ ); //defined here.
		//this method finds the matching score between each sequence in the input sequences and itself
		//and saves it in result array provided in targ parameter. Both forward and reverse scores are found.
		free( targ );
	}
...
//fill res array in arg parameter with scores values resulted from either local or mafft distance scores
//calculated between every two sequences in the input sequences
...
#else
			//this method returns max weight between mseq1[0] and mseq2[0]
			res[j] = L__align11_noalign( n_dis_consweight_multi, mseq1, mseq2 ); //defined in Lalign11.c.
#endif
		}
		else
		{
//			reporterr( "\n\nj=%d, map[j]=%d\n\n", j, map[j] );
			//returns value based on comparisons of table1 and spointt[map[j]] values
			res[j] = localcommonsextet_p2( table1, spointt[map[j]] ); //defined here.
		}
	}
	//free allocated memory
	if( dodp ) // nakuserukamo
...
//	FILE *adfp;
	...
	{
		fprintf( stdout, "Not necessary!\n" ); //then direction adjustment is not necessary, since it is introduced only for DNA sequences
		for( i=0; i<njob; i++ ) {
			fprintf( stdout, "_F_%-10.10s\n", name[i]+1 );
			fprintf( outfp, "_F_%-10.10s\n", name[i]+1 ); //set all sequences as forward
		}
		exit( 1 );
	}
#endif
	...
	if( alg == 'G' ) // dp to the first sequence
	{
		...
		for( i=0; i<istart; i++ )
		{
			...
			name[i][13] = 0; //and end 14-chars name with 0 -null- character
		}
		//by end of for loop above, 'seq' is replaced by 'seq' without gaps and 'name' is replaced by short names with '_F_' at its start
...
//				fprintf( stderr, "REVERSE!!!\n" );
			...
			{
				strcpy( seq[i], mseq2[0] ); //fill seq[i] with forward chars from mseq2[0]
		...
		//each sequence is replaced either by the original forward sequence or the reversed one - based on best matching weight -.
	}
	else if( alg == 'm' ) //what is 'm' algorithm ?   //currently, we call this file with m algorithm
	{
...
//			nthread = 0; // heiretsu keisan no kouritsu ha warui node
		...
		if( dorp == 'd' ) tsize = (int)powf(4, 6); //pow( 4, 6 ); //tsize defined in both defs.h and mltaln.h. I think this means tuple size
		else              tsize = (int)powf(6, 6); //pow( 6, 6 ); // iranai
		...
		if( !dodp ) //if dodp is not set, i.e. approximation option
		{
			if( nadd )
				iend = njob - nadd;
			else
			...
			{
				gappick0( tmpseq, seq[i] ); //copy seq[i] chars to tmpseq without gaps
				strcpy( seq[i], tmpseq ); //copy tmpseq to seq[i], so now seq[i] contains all seq chars without gaps
				make_direction_list_seq_grp_nuc( grpseq, tmpseq ); //defined here. set values from amino_grp array to 'grpseq' based on 'tmpseq' chars
				make_direction_list_makepointtable_nuc( pointt[i], grpseq ); //defined here. set pointt[i] to some value based on calculations on values in grpseq
				spointt[i] = pointt[i];
			}
			//so, after this for loop, we will have all sequences chars updated in seq matrix to be without gaps
			//and also pointt and spointt filled with values calculated based on seq chars and amino_grp values
...
//				makecompositiontable_p( table1, pointt[i] ); -> moto no basho ni modosu
				make_direction_list_seq_grp_nuc( grpseq, revseq ); //set values from amino_grp array to 'grpseq' based on 'revseq' chars
				make_direction_list_makepointtable_nuc( pointt_rev[i], grpseq ); //set pointt_rev[i] to some value based on calculations on values in grpseq
...
//				reporterr( "pointt[i][0] = %p\n", pointt[i][0] );
		...
		if( contrastsort ) // sukoshi chuui //default = 1, and if set from parameters = 0
		{
			...
			if( dodp ) { //accurate option
				//defined here.
				//fills contrastorder array with sorted matching scores calculated from aligning each sequence with itself
				//using local aligning algorithm
				makecontrastorder( contrastorder+istart, seq+istart, iend, istart );
			} else { //approximation option
				//defined here.
				//fills contrastorder array with sorted matching scores calculated from aligning each sequence with itself
				//using mafft-distance
...
//			fprintf( stderr, "\r %d / %d ", i, njob );
			...
			if( !dodp )
			{
				...
				table1_rev = (short *)calloc( tsize, sizeof( short ) );
				if( !table1_rev ) ErrorExit( "Cannot allocate table1_rev\n" );
				//defined in mafft-distance.c. increment table1 values in pointt[ic] indecies
				make_distance_makecompositiontable_p( table1, pointt[ic] );
				//increment table1_rev values in pointt_rev[ic] indecies
				make_distance_makecompositiontable_p( table1_rev, pointt_rev[ic] );
			}
#endif
...
#endif
			{
				thread_arg_t *targ; //defined here.

				if( i%100==1 ) fprintf( stdout, " %d / %d   \r", i, njob );
				targ = (thread_arg_t *) calloc( 1, sizeof( thread_arg_t ) );
				targ[0].iend = iend;
				targ[0].map = map;
				targ[0].seq = seq;
				...
				targ[0].iq = i;  // iranai
				directionthread( targ ); //defined here.
				//fill res array in targ parameter with scores values resulted from either local or mafft distance scores
				//calculated between every two sequences in the input sequences
				free( targ );
			}
...
//			reporterr( "i=%d, res_reverse = %f\n", i, res_reverse );
			...
			else if( mode == 'a' ) //this is the option we currently use
			{
				res_reverse = 0.0;
...
//			if( (res_reverse-res_forward)/res_max > thresholdtorev ) // tekitou
		...
		if( name[0][1] == 'R' ) //if first sequence is reversed, then replace all R with F and vice versa
		{
			for( j=0; j<njob; j++ )
			{
				if( name[j][1] == 'R' ) {
...
//	SHOWVERSION;
	...
	//I think this file finds score of sequences and make them forward or reverse
	//It depends mainly on taking the first sequence and finding the score between it and all next sequences
	//then we compare both forward and reverse scores to determine the better direction (R or F)

	//There are two options for finding score, accurate - which uses local pairwise score -,
	//and the mafft distance one which depends on 6mers score and it is approximate

	//I think we will need the accurate option, since we already works on the accurate LINSi, GINSi and EINSi options
